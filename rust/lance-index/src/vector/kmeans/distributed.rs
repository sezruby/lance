...
mod tests {
    ...
    // ---- full multi-iteration Lloyd-loop parity ------------------------------
    //
    // The tests above cover the primitives in isolation (one E-step, merge/reduce
    // algebra, init). These drive the primitives through a full Lloyd loop — the
    // exact composition the Spark orchestration (`DistributedExternalIndexBuild`)
    // runs — and pin two properties the orchestration relies on:
    //   1. sharding + tree-reduce is invariant (any shard count => same centroids);
    //   2. the loop converges to the same clustering as single-node `KMeans`.

    /// `k` well-separated blobs of `per` points each, at centers `j * SPACING`
    /// (all dims) plus uniform jitter in `[-1, 1]`. SPACING (100) dwarfs the
    /// jitter, so every point's nearest center is its own blob: k-means
    /// assignment is unambiguous and no cluster ever empties. That precondition
    /// is what makes bit-level parity possible — single-node `KMeans` splits a
    /// large cluster to refill an empty one, whereas `finalize_centroids` carries
    /// the previous centroid forward, so the two only trace identical
    /// trajectories when no cluster empties.
    const BLOB_SPACING: f32 = 100.0;

    fn make_blobs(k: usize, per: usize, dim: usize, seed: u64) -> FixedSizeListArray {
        use rand::{Rng, SeedableRng, rngs::StdRng};
        let mut rng = StdRng::seed_from_u64(seed);
        let mut v = Vec::with_capacity(k * per * dim);
        for j in 0..k {
            let center = j as f32 * BLOB_SPACING;
            for _ in 0..per {
                for _ in 0..dim {
                    v.push(center + rng.random_range(-1.0..1.0));
                }
            }
        }
        FixedSizeListArray::try_new_from_values(Float32Array::from(v), dim as i32).unwrap()
    ...
    /// Init centroids offset from each blob center by `offset` (< half the
    /// spacing), so every init row is still closest to its own blob (stable
    /// one-per-blob assignment) yet off the blob mean, forcing the loop to
    /// actually iterate.
    fn perturbed_centers(k: usize, dim: usize, offset: f32) -> FixedSizeListArray {
        let mut v = Vec::with_capacity(k * dim);
        for j in 0..k {
            for _ in 0..dim {
                v.push(j as f32 * BLOB_SPACING + offset);
            }
        }
        FixedSizeListArray::try_new_from_values(Float32Array::from(v), dim as i32).unwrap()
    }

    /// Split `data`'s rows into `parts` contiguous shards (last shard takes the
    /// remainder), mirroring how Spark partitions the cached training sample.
    fn split_fsl(data: &FixedSizeListArray, parts: usize) -> Vec<FixedSizeListArray> {
        ...
        for p in 0..parts {
            let rows = if p == parts - 1 { n - start } else { base };
            let slice = values[start * dim..(start + rows) * dim].to_vec();
            shards.push(
                FixedSizeListArray::try_new_from_values(Float32Array::from(slice), dim as i32)
                    .unwrap(),
            );
            start += rows;
        }
    ...
    /// One driver-orchestrated distributed Lloyd loop over `shards`: broadcast
    /// centroids -> per-shard E-step (`compute_partial_stats`) -> tree-reduce
    /// (`reduce_partial_stats`) -> M-step (`finalize_centroids`), for a fixed
    /// `iters` (no early stop, so the two sides of a parity check run the exact
    /// same number of updates). Returns the final centroids and the last round's
    /// total loss. This is the Rust mirror of the Scala loop in
    /// `DistributedExternalIndexBuild.trainCentroidsDistributed`.
    fn run_distributed_lloyd(
        shards: &[FixedSizeListArray],
        init: &FixedSizeListArray,
        dt: DistanceType,
        iters: usize,
    ) -> (FixedSizeListArray, f64) {
        let mut centroids = init.clone();
        let mut loss = f64::MAX;
        for _ in 0..iters {
            let reduced = reduce_partial_stats(
                shards
                    .iter()
                    .map(|s| compute_partial_stats(&centroids, s, dt).unwrap()),
            )
            .unwrap();
            loss = reduced.total_loss();
            centroids = finalize_centroids(&reduced, &centroids).unwrap();
        }
        (centroids, loss)
    }

    #[test]
    fn test_distributed_lloyd_is_shard_invariant() {
        let (k, per, dim) = (8, 128, 8);
        let data = make_blobs(k, per, dim, 100);
        let init = perturbed_centers(k, dim, 5.0);

        // Same init, same iteration count — only the shard count (hence the
        // reduce order) differs.
        let (c1, l1) =
            run_distributed_lloyd(std::slice::from_ref(&data), &init, DistanceType::L2, 15);
        let (c3, l3) = run_distributed_lloyd(&split_fsl(&data, 3), &init, DistanceType::L2, 15);
        let (c7, l7) = run_distributed_lloyd(&split_fsl(&data, 7), &init, DistanceType::L2, 15);

        let v1 = c1.values().as_primitive::<Float32Type>().values();
        for (label, cn) in [("3-shard", &c3), ("7-shard", &c7)] {
            let vn = cn.values().as_primitive::<Float32Type>().values();
            assert_eq!(v1.len(), vn.len());
            for (x, y) in v1.iter().zip(vn.iter()) {
                assert!(
                    (x - y).abs() < 1e-3,
                    "{label} centroid diverged from single-shard: {x} vs {y}"
                );
            }
        }
        // Loss is a reduce-order-sensitive f64 sum; agree to a tight relative eps.
        for (label, ln) in [("3-shard", l3), ("7-shard", l7)] {
            assert!(
                (l1 - ln).abs() <= 1e-4 * l1.max(1.0),
                "{label} loss diverged: {l1} vs {ln}"
            );
        }
    }

    #[test]
    fn test_distributed_lloyd_matches_single_node_kmeans() {
        use std::sync::Arc;
        let (k, per, dim) = (8, 200, 8);
        let data = make_blobs(k, per, dim, 314);
        let init = perturbed_centers(k, dim, 5.0);
        let iters = 25u32;

        // Distributed loop over 4 shards, from the shared init.
        let (dist_centroids, dist_loss) = run_distributed_lloyd(
            &split_fsl(&data, 4),
            &init,
            DistanceType::L2,
            iters as usize,
        ...
        // Single-node reference from the SAME incremental init over the SAME data
        // (n = 1600 < k*512 = 4096, so `new_with_params` uses every row — no slice).
        let params = KMeansParams::new(Some(Arc::new(init)), iters, 1, DistanceType::L2);
        let km = KMeans::new_with_params(&data, k, &params).unwrap();
        let ref_centroids = km.centroids.as_primitive::<Float32Type>().values();
        let dist_values = dist_centroids
            .values()
            .as_primitive::<Float32Type>()
            .values();

        // Both start one-per-blob and stay there, so row j is blob j's centroid on
        // both sides — no permutation. They must agree, and both must sit on the
        // blob mean (≈ j * SPACING; jitter mean of `per` uniform[-1,1] draws ≈ 0).
        assert_eq!(dist_values.len(), ref_centroids.len());
        for j in 0..k {
            for d in 0..dim {
                let (a, b) = (dist_values[j * dim + d], ref_centroids[j * dim + d]);
                assert!(
                    (a - b).abs() < 1e-1,
                    "centroid[{j}][{d}] distributed {a} vs single-node {b}"
                );
                let truth = j as f32 * BLOB_SPACING;
                assert!(
                    (a - truth).abs() < 0.5,
                    "centroid[{j}][{d}] = {a} did not converge to blob mean ≈ {truth}"
                );
            }
        }

        // total_loss (distributed) and KMeans::loss both sum squared-L2
        // assignment distances via `compute_membership_and_dist`, so once both
        // converge they measure the same clustering — agree within 1%.
        assert!(
            (dist_loss - km.loss).abs() <= 1e-2 * km.loss.max(1.0),
            "distributed loss {dist_loss} vs single-node KMeans loss {}",
            km.loss
        );
    }
}
